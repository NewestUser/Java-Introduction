...
Пример за абстракция от всекидневнието:
...
на тези абстракции (в този случай `Bird` и `Airplane`). Тези конкренти репрезентации, могат да се представят
...
тази ралика, че размера му не е фиксирън. Един `ArrayList` може да бъде итериран, да се добавят нови елементи, да се 
